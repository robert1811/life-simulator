...
const ownedAssets = (type) =>{
    ...
    for(let asset of player.inventory[type]){
        string = string.concat(`<div onclick="functionTemplates.trigger.ownedAssetWindow(this)" data-type="${type}" data-index="${asset.inventoryIndex}" class="cell">
        ${asset.label}
        </div>`)
    } return string
...
const relationShipListifier = (category) => {
   const title = capitalize(category)
    ...
    let index = 0;
    //these bars depend on functionTemplates.handleRelationBars()
